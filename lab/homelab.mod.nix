{ vpn-confinement, sops-nix, ... }:
{
	...
	in
	{
		...
		sops.secrets."koito_subsonic_params" = { };
...
ND_LASTFM_SECRET=${config.sops.placeholder."lastfm/secret"}
		...
		sops.templates."koito.env" = {
			content = ''
KOITO_DEFAULT_THEME=catppuccin
KOITO_LISTEN_PORT=${toString koito_port}
KOITO_SUBSONIC_URL=http://localhost:${toString navidrome_port}/music
KOITO_SUBSONIC_PARAMS=${config.sops.placeholder."koito_subsonic_params"}
			'';
		};
...
# synced works incorrectly, so the order is important (lrclib is the only one that offers synced lyrics)
					...
					*/
					...
					fetchart = {
						...
						minwidth = 1200;
						maxwidth = 1200;
						lastfm_key = config.sops.secrets."lastfm/api_key".path;
					};
			...
			virtualHosts."atlas.volbot.org" = {
				...
				locations."/" = {
					return = ''
						...
						<h3><a href="http://koito.volbot.org">koito</a></h2>
						<h2>advanced</h2>
			...
			tunnels = {
				"14fc4080-e2b1-4328-b111-55ca2b37cab8" = {
					...
					ingress = {
						...
						"koito.volbot.org" = {
							service = "http://127.0.0.1:${toString koito_port}";
						};
						"ssh.volbot.org" = {
...
# Added:
		...
		services.navidrome = {
			...
			settings = {
				...
				LastFM.Enabled = true;
				LastFM.Language = "en";
				ListenBrainz.BaseURL = "http://localhost:${toString koito_port}/apis/listenbrainz/1";
