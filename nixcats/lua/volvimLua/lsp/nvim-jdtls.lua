...
function M.setupJDTLS()
	...
	if ok_mason then
		...
		if nixCats.extra("javaExtras") then
			jdtls_path = nixCats.extra("javaExtras.jdtls")
			lombok_path = nixCats.extra("javaExtras.lombok")
			...
			if jt_path and jda_path then
				jt_server_jars =
					vim.fn.glob(jt_path .. "/share/vscode/extensions/vscjava.vscode-java-test/server/*.jar")
				jda_server_jar = vim.fn.glob(
					jda_path
	...
	local workspace_dir = vim.loop.os_homedir() .. "/.cache/jdtls/workspace/" .. project_name
        -- the following line works, but critically, the file "config.ini" must be copied from jdtls_path to the path below.
        -- by default, it gets put in the nix store for jdtls, which is read-only and causes errors.
        -- there's probably a nix-y way to do this, but it's late and this works.
	...
	local config = {
		...
		cmd = {
			...
			-- 💀
			"-jar",
			vim.fn.glob(jdtls_path .. "/share/java/jdtls/plugins/org.eclipse.equinox.launcher_*.jar"),
			--          ^^^^^^^^^^                                           ^
